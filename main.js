...
<div class="relative bg-[url('/img/background.jpg')] text-white w-screen h-screen flex flex-col justify-center items-center">
  ...
  <div id="headline" class="absolute top-8 left-8 flex flex-col gap-0">
    <h4 class="text-white text-2xl uppercase leading-compact font-medium">Blitz Series</h4>
    <h1 id="title" class="text-7xl uppercase leading-compact font-medium text-red-600" >Portal Lift</h1>
  </div>
  
  <div id="scoreboard" class="absolute top-32 flex flex-col gap-0 text-center">
    <p id="current-score" class="text-white text-7xl uppercase leading-compact font-medium"><span>0</span> Ton</p>
    <p id="message">Click the Pit Lift to <b >Start</b></p>
    <h3 id="heading"></h3>
  </div>
  ...
  <div id="game-powerbar" class="absolute flex align-bottom justify-center left-8 w-24 h-1/2 bg-black bg-opacity-50 rounded-full border border-white overflow-hidden">
    <div id="powerbar-wrapper" class="w-full relative">
      <div id="powerbar" class="absolute bottom-0 left-0 w-full bg-red-600" style="height: var(--game-value) !important;"></div>
    </div>
  </div>
  ...
  <div id="action-buttons" class="absolute bottom-8 left-8 flex gap-4 items-center flex-start">
    <button id="reset" class="bg-white bg-opacity-20 hover:bg-opacity-30 border border-white border-1-solid text-white font-medium text-2xl uppercase h-16 px-4">
    ...
    <p><label for="slider-difficulty">Difficulty:</label> <input type="range" min="1" max="10" value="5" id="slider-difficulty"/></p>
    <p><label for="slider-score-size">Size:</label> <input type="range" min="1" max="500" value="50" id="slider-score-size"/></p>
  </div>
</div>
`
new ClickGame({
